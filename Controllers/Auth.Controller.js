...
module.exports = {
  register: async (req, res, next) => {
    try {
      ...
      // if (!phonenumber || !password) throw createError.BadRequest()
  ...
  login: async (req, res, next) => {
    ...
    try {
      ...
      const message="User Registered successfully"
      const username=user.name_of_user
      res.send({ accessToken, refreshToken,message,username })
    } catch (error) {
      if (error.isJoi === true)
        return next(createError.BadRequest('Invalid Phonenumber/Password'))
      next(error)
    }
  },
